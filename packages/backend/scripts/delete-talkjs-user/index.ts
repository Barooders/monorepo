...
// PRODUCTION
// const appId = 'x3kQChsO';
// const secretKey = 'sk_live_iXoA3s2A2vFr2D9LDj487iywAmvw5n7i';

// STAGING
const appId = 'teZDC2o0';
const secretKey = 'sk_test_dssuZgukJxwCsR4nd9l4QxBJr9YmdIB7';

// userIds is the id that you pass in `Talk.User({ id: <USER_ID> })` code.
const userIds = ['6731635523825'];
...
const cleanupOnlyUsersMessages = false;
...
async function anynomizeUsersData(app: App, userIds: string[]) {
  userIds.forEach(async (userId) => {
    ...
    const conversations = await listResources({
      app: app,
      resource: `users/${userId}/conversations`,
      limit: 100,
    });
      ...
      for (const message of messages) {
        ...
        if (cleanupOnlyUsersMessages) {
          if (message.senderId === userId) {
            await put({
              app,
              resource: `conversations/${conversation.id}/messages/${message.id}`,
              record: m,
            });
          }
        }
...
async function listResources({
  ...
  const path =
    host + `/v1/${app.id}/${resource}?limit=${limit}${paginateMaybe}`;
...
async function doRequest(
  ...
  if (response.status === 200) {
    ...
    console.warn(
      '[TalkJS]',
      `Couldn't get ${verb} ${path}. Process it manually. Error: ${error}`,
    );
    return [];
  }
}

void anynomizeUsersData(sourceApp, userIds);
